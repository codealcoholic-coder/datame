...
export default function AdminPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/posts?id=${id}&admin_key=${adminKey}`, {
      method: "DELETE",
    });
    await refreshPosts();
  };
  ...
  const handleCommentAction = async (
    ...
    if (action === "delete") {
      await fetch(`/api/comments?id=${id}&admin_key=${adminKey}`, {
        method: "DELETE",
      });
    } else {
  ...
  if (loading)
    return (
      ...
      >
        <span style={{ color: "var(--muted-custom)" }}>Loading...</span>
      </div>
    );
  if (authorized === false)
    return (
      ...
      >
        ...
        <div style={{ fontSize: "0.85rem", color: "var(--muted-custom)" }}>
          Invalid admin key.
        </div>
      </div>
    );
  ...
  const tabBtnStyle = (active: boolean): React.CSSProperties => ({
    ...
    color: active ? "var(--bg)" : "var(--muted-custom)",
    border: "1px solid var(--border-custom)",
    padding: "10px 24px",
    fontFamily: "var(--font-sans)",
    fontSize: "0.75rem",
  ...
  const actionBtn = (bg: string): React.CSSProperties => ({
    background: bg,
    color: bg === "transparent" ? "var(--muted-custom)" : "var(--bg)",
    border: `1px solid ${bg === "transparent" ? "var(--border-custom)" : bg}`,
    padding: "6px 14px",
    fontFamily: "var(--font-sans)",
    fontSize: "0.7rem",
    fontWeight: 500,
                  ...
                  >
                        ...
                        <span
                          style={{
                            ...
                            background: post.published
                              ? "rgba(71,255,232,0.12)"
                              : "rgba(255,107,71,0.12)",
                            color: post.published ? "#47ffe8" : "#ff6b47",
                            border: `1px solid ${post.published ? "rgba(71,255,232,0.3)" : "rgba(255,107,71,0.3)"}`,
                            letterSpacing: "0.1em",
                            textTransform: "uppercase",
                          }}
                        >
                    ...
                    <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                      <button
                        ...
                        style={actionBtn("transparent")}
                      >
                        {post.published ? "Unpublish" : "Publish"}
                      </button>
                      <button
                        onClick={() => openEditor(post)}
                        style={actionBtn("#47ffe8")}
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(post._id)}
                        style={actionBtn("#ff6b47")}
                      >
                        Delete
                      </button>
                    </div>
                ...
                {comments.map((c) => (
                  ...
                  >
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            color: c.approved ? "#47ffe8" : "#e8ff47",
                            border: `1px solid ${c.approved ? "rgba(71,255,232,0.3)" : "rgba(255,199,71,0.3)"}`,
                            letterSpacing: "0.1em",
                            textTransform: "uppercase",
                          }}
                        >
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {!c.approved && (
                        <button
                          onClick={() => handleCommentAction(c._id, "approve")}
                          style={actionBtn("#47ffe8")}
                        >
                          ✓ Approve
                        </button>
                      ...
                      {c.approved && (
                        <button
                          onClick={() => handleCommentAction(c._id, "reject")}
                          style={actionBtn("transparent")}
                        >
                          ✗ Reject
                        </button>
                      ...
                      <button
                        onClick={() => handleCommentAction(c._id, "delete")}
                        style={actionBtn("#ff6b47")}
                      >
                        🗑 Delete
                      </button>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setForm({
                        ...
                        categoryColor: cat?.color || "#e8ff47",
                      });
                    }}
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8, paddingTop: 4 }}>
                    <button
                      ...
                      style={tabBtnStyle(!form.published)}
                    >
                      Draft
                    </button>
                    <button
